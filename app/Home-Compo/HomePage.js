"use client";

import {
  Calculator,
  Calendar,
  Clock,
  Scissors,
  Ruler,
  Scale,
} from "lucide-react";

import { Button } from "@/components/ui/button";
import Link from "next/link";
import { useState } from "react";
import { AgeCal } from "../tools-compo/AgeCal";

export function HomePage() {
  const [activeTool, setActiveTool] = useState("age-calculator");

  const tools = [
    {
      id: "age-calculator",
      title: "Age Calculator",
      description: "Calculate exact age between two dates",
      icon: <Calendar className="w-6 h-6" />,
      component: <AgeCal />,
    },
    {
      id: "time-calculator",
      title: "Time Calculator",
      description: "Add or subtract time values",
      icon: <Clock className="w-6 h-6" />,
      component: (
        <div className="p-8 text-center">Time Calculator - Coming Soon</div>
      ),
    },
    {
      id: "unit-converter",
      title: "Unit Converter",
      description: "Convert between different measurement units",
      icon: <Ruler className="w-6 h-6" />,
      component: (
        <div className="p-8 text-center">Unit Converter - Coming Soon</div>
      ),
    },
    {
      id: "bmi-calculator",
      title: "BMI Calculator",
      description: "Calculate your Body Mass Index",
      icon: <Scale className="w-6 h-6" />,
      component: (
        <div className="p-8 text-center">BMI Calculator - Coming Soon</div>
      ),
    },
  ];

  return (
    <div className="bg-gradient-to-b from-gray-50 to-white">
      {/* Features Section */}
      <section className="py-12 px-4 sm:px-6 lg:px-8 bg-gray-50">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-3xl font-bold text-center mb-12">
            Why Choose Our Tools?
          </h2>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="text-center">
              <div className="bg-blue-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                <Calculator className="w-8 h-8 text-blue-600" />
              </div>
              <h3 className="text-xl font-semibold mb-2">Easy to Use</h3>
              <p className="text-gray-600">
                Intuitive interfaces designed for everyone from students to
                professionals.
              </p>
            </div>

            <div className="text-center">
              <div className="bg-blue-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                <Scissors className="w-8 h-8 text-blue-600" />
              </div>
              <h3 className="text-xl font-semibold mb-2">No Installation</h3>
              <p className="text-gray-600">
                Access all tools directly in your browser with no downloads
                required.
              </p>
            </div>

            <div className="text-center">
              <div className="bg-blue-100 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4">
                <Clock className="w-8 h-8 text-blue-600" />
              </div>
              <h3 className="text-xl font-semibold mb-2">Save Time</h3>
              <p className="text-gray-600">
                Get instant results without manual calculations or complex
                formulas.
              </p>
            </div>
          </div>
        </div>
      </section>

      {/* Call to Action */}
      <section className="py-16 px-4 sm:px-6 lg:px-8 bg-blue-600 text-white">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-4">
            Ready to Simplify Your Tasks?
          </h2>
          <p className="text-xl mb-8 opacity-90">
            Explore our full collection of tools designed to make your daily
            calculations effortless.
          </p>
          <Button asChild variant="secondary" size="lg">
            <Link href="#tools">Browse All Tools</Link>
          </Button>
        </div>
      </section>
    </div>
  );
}

...
const FALLBACK = {
  USD:1,     BDT:120.45, EUR:0.92,  GBP:0.79,  JPY:157.3, AUD:1.5,
  CAD:1.37,  CHF:0.89,   CNY:7.25,  INR:83.5,  SAR:3.75,  AED:3.67,
  PKR:278.5, SGD:1.35,   NZD:1.63,  HKD:7.8,   SEK:10.5,  NOK:10.6,
  DKK:6.87,  TRY:32.5,   RUB:89,    BRL:5.35,  ZAR:18.2,  MXN:18.4,
  PHP:58.7,  MYR:4.71,   IDR:16400, THB:36.7,  KRW:1380,  KWD:0.31,
  BHD:0.38,  OMR:0.38,   QAR:3.64,  EGP:47.8,  LKR:302,   NPR:133.6,
};
...
const COMPARE_CODES = ["EUR","GBP","JPY","INR","AUD","CAD","SAR","AED","SGD","PKR","CNY","CHF"];

const NOTE_MAP = {
  USD:[100,50,20,10,5,1], EUR:[200,100,50,20,10,5],
  GBP:[50,20,10,5],       BDT:[1000,500,200,100,50,20,10,5,2,1],
  INR:[500,200,100,50,20,10], SAR:[500,100,50,10,5,1],
  AED:[500,200,100,50,20,10,5], JPY:[10000,5000,1000],
};
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0 text-left">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{value.name}</p>
          </div>
        </div>
        <ChevronDown
        ...
        {open && (
          ...
          >
            <div className="p-3 border-b border-gray-100 dark:border-gray-800 flex items-center gap-2.5 bg-gray-50 dark:bg-gray-800/60">
              ...
              />
              {q && (
                <button onClick={() => setQ("")} className="text-gray-400 hover:text-gray-600 cursor-pointer">
                  ×
                </button>
              )}
            </div>
            <div className="max-h-64 overflow-y-auto">
              {filtered.length === 0 ? (
                <p className="text-center py-6 text-xs text-gray-400">No results</p>
              ) : (
                filtered.map((c) => (
                  <button
                    key={c.code}
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2.5 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer border-b border-gray-50 dark:border-gray-800/60 last:border-0 ${
                      value.code === c.code ? "bg-brandColor/5 dark:bg-brandColor/10" : ""
                    }`}
                  >
                    <div className="flex items-center gap-3">
                      <span className="w-7 h-4.5 rounded overflow-hidden shadow-xs shrink-0">
                        <ReactCountryFlag countryCode={c.country} svg style={{ objectFit: "cover" }} />
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      {value.code === c.code && <Check className="w-3.5 h-3.5 text-brandColor" />}
                      <span className="text-xs font-semibold text-gray-400">{c.symbol}</span>
                    </div>
                  </button>
                ))
              )}
...
export default function CurrencyConverter() {
  ...
  const fetchRates = async (base) => {
    ...
    try {
      ...
      if (data.result === "success") {
        ...
        setLastUp(d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
  ...
  useEffect(() => { fetchRates(from.code); }, [from]);

  // ── calculations ───────────────────────────────────────────────────────────
  const calc = useMemo(() => {
    const raw     = parseFloat(amount) || 0;
    const rawRate = rates[to.code] ?? (FALLBACK[to.code] / FALLBACK[from.code] ?? 1);
    const netRate = rawRate * (1 - markup.value);
    const fee     = raw * markup.value;
    return { raw, rawRate, netRate, converted: raw * netRate, fee, inverse: netRate > 0 ? 1 / netRate : 0 };
  ...
  const notes = useMemo(() => {
    ...
    for (const d of denoms) {
      if (rem >= d) { rows.push({ d, n: Math.floor(rem / d) }); rem %= d; }
    }
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-2xl dark:shadow-gray-900/50 overflow-hidden border border-gray-100 dark:border-gray-700/60">
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-violet-200 mt-1 text-sm sm:text-base max-w-lg">
                    Live interbank rates · 36 major currencies · Bank markup simulator · Cash note estimator
                  </p>
                </div>
              </div>
              ...
              >
                <span
                  className={`w-2 h-2 rounded-full ${
                    loading ? "bg-white animate-pulse" : hasError ? "bg-amber-400" : "bg-emerald-400"
                  }`}
                />
                {loading
                  ? "Fetching rates…"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-gray-100 dark:divide-gray-700/60">
            ...
            <div className="lg:col-span-8 p-6 sm:p-8 space-y-6">
              ...
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700/60 pb-4">
                ...
                <button
                  onClick={() => { setAmount("1000"); setFrom(CURRENCIES[0]); setTo(CURRENCIES[1]); setMarkup(MARKUP_FEES[0]); setPrecision(2); }}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-3 items-end">
                ...
                {/* Swap button */}
                <div className="flex justify-center items-end pb-2.5">
                  <button
                    type="button"
                    onClick={handleSwap}
                    title="Swap currencies"
                    className="w-10 h-10 flex items-center justify-center rounded-full bg-brandColor hover:bg-brandColorHover text-white shadow-lg shadow-brandColor/30 hover:scale-110 active:scale-95 transition-all duration-200 cursor-pointer"
                  >
                    <motion.div
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 pt-0.5">
                    1 {from.code} = <strong className="text-gray-600 dark:text-gray-300">{calc.netRate.toFixed(4)} {to.code}</strong>
                    &ensp;·&ensp;
                    1 {to.code} = <strong className="text-gray-600 dark:text-gray-300">{calc.inverse.toFixed(5)} {from.code}</strong>
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 relative">
                <div className="relative">
                  <CurrencyPicker
                    ...
                    value={from}
                    open={fromOpen}
                    onToggle={() => { setFromOpen((p) => !p); setToOpen(false); }}
                    onSelect={(c) => { setFrom(c); setFromOpen(false); }}
                    wrapRef={fromRef}
                  />
                </div>
                <div className="relative">
                  <CurrencyPicker
                    ...
                    value={to}
                    open={toOpen}
                    onToggle={() => { setToOpen((p) => !p); setFromOpen(false); }}
                    onSelect={(c) => { setTo(c); setToOpen(false); }}
                    wrapRef={toRef}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                >
                  ...
                  <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${showAdv ? "rotate-180" : ""}`} />
                </button>
              </div>
                  ...
                  >
                    <div className="space-y-5 pt-1">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-5 bg-gray-50 dark:bg-gray-900/40 rounded-2xl border border-gray-100 dark:border-gray-700/60">
                        <div className="space-y-2">
                          ...
                          <div className="relative">
                            <select
                              value={markup.id}
                              onChange={(e) =>
                                setMarkup(MARKUP_FEES.find((f) => f.id === e.target.value) || MARKUP_FEES[0])
                              }
                          ...
                          {markup.value > 0 && (
                            <div className="flex items-center justify-between text-xs bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/40 text-amber-700 dark:text-amber-400 rounded-lg px-3 py-2 mt-1">
                              ...
                              <strong>{from.symbol}{fmt(calc.fee)}</strong>
                            </div>
                          )}
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-xs text-gray-400 dark:text-gray-500 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg px-3 py-2 font-mono mt-1">
                            Result preview:{" "}
                            <span className="font-bold text-gray-800 dark:text-white">{to.symbol}{fmt(calc.converted, precision)}</span>
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="p-5 bg-gray-50 dark:bg-gray-900/40 rounded-2xl border border-gray-100 dark:border-gray-700/60 space-y-4">
                        ...
                        <div className="relative">
                          <svg className="w-full" style={{ height: 120 }} viewBox="0 0 560 110" preserveAspectRatio="none">
                            <defs>
                              <linearGradient id="cGrad" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="0%" stopColor="#7c00fe" stopOpacity={chart.trend === "up" ? "0.22" : "0.12"} />
                                <stop offset="100%" stopColor="#7c00fe" stopOpacity="0.00" />
                              </linearGradient>
                            </defs>
                            {[22, 55, 88].map((y) => (
                            ...
                            <path d={chart.line} fill="none" stroke="#7c00fe" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                            {[0, 14, 29].map((i) => {
                              const d = history[i];
                              if (!d) return null;
                              const x = (i / 29) * 560;
                              const y = 110 - ((d.rate - chart.min) / (chart.max - chart.min || 1)) * 110;
                              return (
                                <g key={i}>
                                  <circle cx={x} cy={y} r="4" fill="#7c00fe" />
                                  <circle cx={x} cy={y} r="7" fill="none" stroke="white" strokeWidth="1.5" className="dark:stroke-gray-800" />
                                </g>
                              );
                            })}
                          </svg>
                          ...
                          <div className="absolute right-0 top-0 flex flex-col justify-between h-full text-[9px] text-gray-400 dark:text-gray-600 font-mono pointer-events-none py-1">
                            ...
                            <span>{((chart.max + chart.min) / 2).toFixed(4)}</span>
            ...
            <div className="lg:col-span-4 p-6 sm:p-8 space-y-6 bg-gray-50/60 dark:bg-gray-900/20">
              ...
              <div className="space-y-3">
                <p className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500">Result</p>
                <div className="bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700/60 shadow-sm space-y-3">
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2.5">
                      <span className="w-8 h-5.5 rounded-md overflow-hidden border border-gray-200 dark:border-gray-700 shrink-0 shadow-sm">
                        <ReactCountryFlag countryCode={from.country} svg style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </span>
                      <span className="font-semibold text-gray-500 dark:text-gray-400">{from.code}</span>
                    </div>
                    <span className="font-extrabold text-gray-800 dark:text-gray-200 tabular-nums">
                      {from.symbol}{fmt(calc.raw)}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                      <span className="w-8 h-5.5 rounded-md overflow-hidden border border-gray-200 dark:border-gray-700 shrink-0 shadow-sm">
                        <ReactCountryFlag countryCode={to.country} svg style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </span>
                      <span className="font-bold text-brandColor">{to.code}</span>
                    </div>
                  ...
                  <div className="pt-2 border-t border-gray-100 dark:border-gray-700/60 space-y-1 text-xs text-gray-400 dark:text-gray-500">
                    <div className="flex justify-between">
                      ...
                      <strong className="text-gray-700 dark:text-gray-300 tabular-nums">{calc.netRate.toFixed(5)} {to.code}</strong>
                    </div>
                    <div className="flex justify-between">
                      <span>1 {to.code}</span>
                      <strong className="text-gray-700 dark:text-gray-300 tabular-nums">{calc.inverse.toFixed(6)} {from.code}</strong>
                    </div>
                    {markup.value > 0 && (
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  {COMPARE_CODES.filter((c) => c !== from.code && c !== to.code).slice(0, 8).map((code) => {
                    const cur = CURRENCIES.find((x) => x.code === code);
                    if (!cur) return null;
                    const r   = rates[code] ?? (FALLBACK[code] / FALLBACK[from.code]);
                      ...
                      >
                        <div className="flex items-center gap-2.5">
                          <span className="w-6 h-4 rounded overflow-hidden shadow-xs border border-gray-200 dark:border-gray-700 shrink-0">
                            <ReactCountryFlag countryCode={cur.country} svg style={{ objectFit: "cover" }} />
                          </span>
                          <span className="text-xs font-bold text-gray-600 dark:text-gray-300">{cur.code}</span>
                        </div>
                        <span className="text-sm font-extrabold text-gray-800 dark:text-white tabular-nums">
                          {cur.symbol}{fmt(val, 2)}
                        </span>
                      </div>
                    );
                  })}
              ...
              {showAdv && (
                <div className="space-y-3">
                  ...
                  <div className="space-y-1.5">
                    {notes.rows.length === 0 ? (
                      <p className="text-xs text-gray-400 text-center py-4">Amount too small for notes</p>
                    ) : (
                      <>
                        {notes.rows.map(({ d, n }) => (
                          <div
                          ...
                          >
                            <span className="flex items-center gap-2 text-xs font-semibold text-gray-600 dark:text-gray-300">
                              ...
                              {to.symbol}{d.toLocaleString()}
                            </span>
                            <span className="text-sm font-black text-gray-800 dark:text-white">×{n}</span>
                          </div>
                        ))}
                        {notes.leftover > 0 && (
                          <p className="text-right text-[10px] text-gray-400 pt-1">
                            Coin change: {to.symbol}{notes.leftover.toFixed(2)}
                          </p>
                        )}
                      </>
                    )}
          ...
          <div className="border-t border-gray-100 dark:border-gray-700/60 px-6 sm:px-10 py-6 bg-gray-50/40 dark:bg-gray-900/20">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-bold text-gray-700 dark:text-gray-300">About This Tool</h2>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
              {[
                { title: "Live Exchange Rates", body: "Rates are fetched from open.er-api.com on page load and whenever you change the source currency. Refresh manually by clicking the status pill in the header." },
                { title: "Markup / Service Fee", body: "Banks and payment providers add a hidden spread (1–5%) on top of interbank rates. Use the advanced option to simulate your real-world conversion cost." },
                { title: "Rate Trend Chart", body: "The 30-day chart is a seeded simulation for illustrative purposes. For real historical rates, consult a financial data provider like Bloomberg or Reuters." },
              ].map(({ title, body }) => (
                <div key={title} className="space-y-1">
                  <p className="font-bold text-gray-700 dark:text-gray-200 text-xs">{title}</p>
                  <p>{body}</p>
                </div>
              ))}
            </div>

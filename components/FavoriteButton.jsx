  ...
  return (
    <button
      ...
      className={`relative p-2.5 rounded-xl border transition-all duration-300 cursor-pointer flex items-center justify-center group/fav ${
        favorited
          ? "bg-amber-50 border-amber-200 text-amber-500 hover:bg-amber-100 hover:border-amber-300"
          : "bg-white/80 border-gray-200 text-gray-400 hover:text-gray-600 hover:border-gray-300 hover:bg-white"
      } ${className}`}
      aria-label={favorited ? "Remove from favorites" : "Add to favorites"}
    >
      <motion.div
        animate={favorited ? { scale: [1, 1.25, 1], rotate: [0, 15, -15, 0] } : { scale: 1 }}
